...
import be.spf.archive.service.Document;
import be.spf.archive.service.DocumentMetadata;
...
public class FileSystemDocumentDao implements IDocumentDao {
     ...
     * 
     * @see be.spf.archive.dao.IDocumentDao#insert(be.spf.archive.service.Document)
     */
    @Override
     ...
     * the parameter.
     * 
     * @see be.spf.archive.dao.IDocumentDao#findByRegistrationNumberDate(java.lang.String, java.util.Date)
     */
    @Override
    public List<DocumentMetadata> findByRegistrationNumberDate(String registrationNumber, Date date) {
        try {
            return findInFileSystem(registrationNumber,date);
        } catch (IOException e) {
            String message = "Error while finding document, registrationNumber: " + registrationNumber + ", date:" + date;
            LOG.error(message, e);
            throw new RuntimeException(message, e);
        }
    }
     ...
     * 
     * @see be.spf.archive.dao.IDocumentDao#load(java.lang.String)
     */
    @Override
    ...
    private List<DocumentMetadata> findInFileSystem(String registrationNumber, Date date) throws IOException  {
        ...
        for (String uuid : uuidList) {
            ...
            if(isMatched(metadata, registrationNumber, date)) {
    ...
    private boolean isMatched(DocumentMetadata metadata, String registrationNumber, Date date) {
        if(metadata==null) {
            return false;
        }
        boolean match = true;
        if(registrationNumber!=null) {
            match = (registrationNumber.equals(metadata.getRegistrationNumber()));
        }
        if(match && date!=null) {
